class GalleryController < ApplicationController
  def index
    @products_gallery=Store.all
  end

  def search
    keyword = '%' + params[:keyword] + '%'
    @products_gallery = Store.find_by_sql ["Select * from stores WHERE name like ? or category like ? or ingredients like ? or description like ?",keyword,keyword,keyword,keyword]
  end
